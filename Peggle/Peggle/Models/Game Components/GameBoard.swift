...
class GameBoard {
    ...
    func configWorldBounds(_ worldDim: CGSize) {
        ...
        let topWall = Wall(dim: CGSize(width: worldDim.width, height: wallThickness), position: Vector2(x: worldDim.width / 2, y: -wallThickness / 2))
        let rightWall = Wall(dim: CGSize(width: wallThickness, height: worldDim.height), position: Vector2(x: worldDim.width + wallThickness / 2, y: worldDim.height / 2))
        let leftWall = Wall(dim: CGSize(width: wallThickness, height: worldDim.height), position: Vector2(x: -wallThickness / 2, y: worldDim.height / 2))
...
class Wall: RigidBody {
    init(dim: CGSize, position: Vector2) {
        super.init(isDynamic: false, material: Material.staticMaterial, collider: BoxCollider(halfWidth: dim.width / 2, halfHeight: dim.height / 2), transform: Transform(position))
    }
}
